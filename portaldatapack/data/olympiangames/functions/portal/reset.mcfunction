scoreboard objectives add slot dummy
scoreboard objectives add geldrop minecraft.dropped:minecraft.crossbow
scoreboard objectives add numbers dummy
scoreboard objectives add onportal dummy
scoreboard objectives add config trigger
scoreboard players set @a config 1
scoreboard objectives add gelconfig dummy
scoreboard objectives add gelconfig2 dummy
scoreboard objectives add iceconfig dummy
scoreboard players set #4 numbers 4
scoreboard players set #3 numbers 3
scoreboard players set #1 numbers 1
scoreboard players set #2 numbers 2
scoreboard players set #5 numbers 5
scoreboard players set #6 numbers 6
summon armor_stand ~ ~ ~ {UUID:[I;46366,35636,32452,12614],CustomNameVisible:0b,NoGravity:1b,Invisible:1b,CustomName:'{"text":"blue"}'}
summon armor_stand ~ ~ ~ {UUID:[I;93572,34563,23457,91345],CustomNameVisible:0b,NoGravity:1b,Invisible:1b,CustomName:'{"text":"orange"}'}
summon armor_stand ~ ~ ~ {UUID:[I;34572,93635,24362,78341],CustomNameVisible:0b,NoGravity:1b,Invisible:1b,CustomName:'{"text":"bluetp"}'}
summon armor_stand ~ ~ ~ {UUID:[I;45642,745312,37563,25835],CustomNameVisible:0b,NoGravity:1b,Invisible:1b,CustomName:'{"text":"orangetp"}'}