execute as @a if score @s slot matches 5 at @e[type=minecraft:arrow,nbt={inGround:1b}] run tp @e[type=minecraft:armor_stand,name="blue"] ~ ~ ~
execute as @a if score @s slot matches 3 at @e[type=minecraft:arrow,nbt={inGround:1b}] run tp @e[type=minecraft:armor_stand,name="orange"] ~ ~ ~
